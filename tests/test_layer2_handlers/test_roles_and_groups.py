...
class TestListSpaceUsers:
    async def test_list_space_users_required_only(self, client, mock_api):
        route = mock_api.get("/spaces/1/users").mock(
            return_value=Response(200, json=[])
        )
        result = await TOOLS["kaiten_list_space_users"]["handler"](
            client, {"space_id": 1}
        )
        assert route.called
        assert result == []
...
class TestGetCompanyGroup:
    async def test_get_company_group_required_only(self, client, mock_api):
        ...
        result = await TOOLS["kaiten_get_company_group"]["handler"](
            client, {"group_uid": "g1"}
        )
        assert route.called
...
class TestUpdateCompanyGroup:
    async def test_update_company_group_required_only(self, client, mock_api):
        ...
        result = await TOOLS["kaiten_update_company_group"]["handler"](
            client, {"group_uid": "g1"}
        )
        assert route.called
        body = json.loads(route.calls[0].request.content)
