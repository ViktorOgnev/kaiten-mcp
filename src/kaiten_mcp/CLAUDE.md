...
MCP-сервер для Kaiten API. 334 инструмента: карточки, доски, проекты, документы, автоматизации, Service Desk, каталоги, SCIM и др.
...
make http-build  # Собрать HTTP-образ
...
```bash
docker build --target baked-http -t kaiten-mcp-http:latest /path/to/kaiten-mcp
docker run --rm \
  --read-only --tmpfs /tmp:noexec,nosuid,size=64m \
  --security-opt=no-new-privileges:true --cap-drop=ALL \
  -p 8000:8000 \
...
Когда MCP-сервер подключён и Claude Code перезапущен, доступны 334 инструмента. Типичные сценарии:

Entity-oriented parity с `kaiten-cli` включает company users, user roles, group
admins/entities, tree-entities, catalog/custom-directory сущности, document
schemas и SCIM v2. Stateful/cache/runtime-фичи CLI не включены по умолчанию:
для multitenant MCP общий кэш допустим только как отдельная явная настройка с
изоляцией по пользователю/tenant.
